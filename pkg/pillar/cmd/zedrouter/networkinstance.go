...
func handleNetworkInstanceCreate(
	...
	status.DhcpRange.Size = getAppCountOnUNet(ctx, &status)
...
func handleNetworkInstanceDelete(ctxArg interface{}, key string,
	...
	appNumOnUNetBaseDelete(status.UUID)
...
func lookupOrAllocateIPv4(status *types.NetworkInstanceStatus,
	...
	networkID := status.UUIDandVersion.UUID
	// if it exists
	if ip, ok := status.IPAssignments[mac.String()]; ok {
		log.Functionf("lookupOrAllocateIPv4(%s-%s): found %s for %s\n",
			networkID.String(), appID.String(), ip.String(), mac.String())
